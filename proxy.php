...
$is_allowed_origin = $origin === 'https://guvenfinans.az' ||
    $origin === 'https://www.guvenfinans.az' ||
...
foreach ($incoming_headers as $key => $value) {
    ...
    if ($normalized_key === 'host') continue;
    if ($normalized_key === 'content-length') continue;
    if ($normalized_key === 'authorization') $has_authorization_header = true;
    $headers[] = "$key: $value";
}

// Some PHP/FastCGI configurations omit Authorization from getallheaders().
...
foreach ($header_lines as $header_line) {
    ...
    $normalized_response_header = strtolower($header_line);
    if (strpos($normalized_response_header, 'http/') === 0 ||
        strpos($normalized_response_header, 'access-control-allow-origin:') === 0 ||
        strpos($normalized_response_header, 'access-control-allow-credentials:') === 0 ||
        strpos($normalized_response_header, 'access-control-allow-methods:') === 0 ||
        strpos($normalized_response_header, 'access-control-allow-headers:') === 0 ||
        strpos($normalized_response_header, 'access-control-max-age:') === 0 ||
        strpos($normalized_response_header, 'vary:') === 0 ||
        strpos($normalized_response_header, 'connection:') === 0 ||
        strpos($normalized_response_header, 'keep-alive:') === 0 ||
        strpos($normalized_response_header, 'transfer-encoding:') === 0) {
        continue;
    }
