...
class ProfileApp {
                                    ...
                                    <div class="notes-editor-toolbar" aria-label="Mətn formatlama paneli">
                                        <div class="notes-color-control" title="Hərfin rəngi">
                                            ...
                                            <div id="noteColorPopover" class="note-color-popover hidden" role="dialog" aria-label="Yazı rəngi seçimi">
                                                <div id="noteColorPickerMain" class="note-color-picker-main" aria-label="Əsas rəng seçimi">
                                                    <span id="noteColorPickerHandle" class="note-color-picker-handle" aria-hidden="true"></span>
                                                </div>
                                                <div class="note-color-controls">
                                                    <span id="noteColorPreview" class="note-color-preview" aria-hidden="true" style="background-color:#111827"></span>
                                                    <input id="noteColorHue" class="note-color-hue" type="range" min="0" max="360" value="220" aria-label="Hue rəng sürüşdürməsi" />
                                                </div>
                                                <div class="note-color-rgb-row" aria-label="RGB dəyərləri">
                                                    <label>R<input id="noteColorRgbR" type="number" min="0" max="255" value="17" inputmode="numeric" /></label>
                                                    <label>G<input id="noteColorRgbG" type="number" min="0" max="255" value="24" inputmode="numeric" /></label>
                                                    <label>B<input id="noteColorRgbB" type="number" min="0" max="255" value="39" inputmode="numeric" /></label>
                                                </div>
                                                <input id="noteTextColor" class="note-color-hidden-input" type="hidden" value="#111827" />
    ...
    async openCreateProtocolModal() {
        ...
        try {
            const draft = await this.getProtocolService().startProtocol({
                title: 'Yeni Protokol',
                description: '',
            ...
            try {
                // ✅ DÜZƏLİŞ: protokolun iştirakçıları yox, ŞİRKƏTİN BÜTÜN ƏMƏKDAŞLARI gətirilir
    ...
    bindStandaloneNoteToolbar() {
        ...
        updateToolbarActiveState();
        const clampColorValue = (value) => Math.max(0, Math.min(255, Number.parseInt(value, 10) || 0));
        const componentToHex = (value) => clampColorValue(value).toString(16).padStart(2, '0');
        ...
        const hexToRgb = (hex) => {
            ...
            return { r: Number.parseInt(full.slice(0, 2), 16) || 0, g: Number.parseInt(full.slice(2, 4), 16) || 0, b: Number.parseInt(full.slice(4, 6), 16) || 0 };
        };
        ...
        const colorPickerState = { ...rgbToHsv(hexToRgb(this.noteFormatState.color || '#111827')) };
        const syncColorPickerUi = (color, syncHsv = true) => {
            const rgb = hexToRgb(color);
            if (syncHsv) Object.assign(colorPickerState, rgbToHsv(rgb));
            const colorInput = document.getElementById('noteTextColor');
            const preview = document.getElementById('noteColorPreview');
            const main = document.getElementById('noteColorPickerMain');
            const handle = document.getElementById('noteColorPickerHandle');
            const hue = document.getElementById('noteColorHue');
            const rInput = document.getElementById('noteColorRgbR');
            const gInput = document.getElementById('noteColorRgbG');
            const bInput = document.getElementById('noteColorRgbB');
            if (colorInput) colorInput.value = color;
            if (preview) preview.style.backgroundColor = color;
            if (main) main.style.backgroundColor = `hsl(${colorPickerState.h}, 100%, 50%)`;
            if (handle) { handle.style.left = `${colorPickerState.s * 100}%`; handle.style.top = `${(1 - colorPickerState.v) * 100}%`; handle.style.backgroundColor = color; }
            if (hue) hue.value = String(Math.round(colorPickerState.h));
            if (rInput) rInput.value = rgb.r;
            if (gInput) gInput.value = rgb.g;
            if (bInput) bInput.value = rgb.b;
        };
        const applyTextColor = (color) => {
            const normalizedColor = rgbToHex(hexToRgb(color));
            syncColorPickerUi(normalizedColor);
            document.getElementById('noteEditorContent')?.focus();
            document.execCommand('foreColor', false, normalizedColor);
            this.noteFormatState = { ...this.noteFormatState, color: normalizedColor };
            updateToolbarActiveState();
        ...
        const pickColorFromMain = (event) => {
            const picker = document.getElementById('noteColorPickerMain');
            if (!picker) return;
            const rect = picker.getBoundingClientRect();
            colorPickerState.s = Math.max(0, Math.min(1, (event.clientX - rect.left) / rect.width));
            colorPickerState.v = 1 - Math.max(0, Math.min(1, (event.clientY - rect.top) / rect.height));
            applyTextColor(rgbToHex(hsvToRgb(colorPickerState)));
        };
        const colorPickerMain = document.getElementById('noteColorPickerMain');
        colorPickerMain?.addEventListener('mousedown', (event) => {
            event.preventDefault();
            pickColorFromMain(event);
            const handleMove = (moveEvent) => pickColorFromMain(moveEvent);
            const stopMove = () => { document.removeEventListener('mousemove', handleMove); document.removeEventListener('mouseup', stopMove); };
            document.addEventListener('mousemove', handleMove);
            document.addEventListener('mouseup', stopMove);
        });
        document.getElementById('noteColorHue')?.addEventListener('input', (event) => {
            colorPickerState.h = Number(event.target.value) || 0;
            applyTextColor(rgbToHex(hsvToRgb(colorPickerState)));
        });
        ['noteColorRgbR', 'noteColorRgbG', 'noteColorRgbB'].forEach(id => {
            document.getElementById(id)?.addEventListener('input', () => {
                applyTextColor(rgbToHex({
                    r: document.getElementById('noteColorRgbR')?.value,
                    g: document.getElementById('noteColorRgbG')?.value,
                    b: document.getElementById('noteColorRgbB')?.value
                }));
            });
        });
        syncColorPickerUi(this.noteFormatState.color || '#111827');
        document.querySelectorAll('.note-color-swatch').forEach(swatch => {
