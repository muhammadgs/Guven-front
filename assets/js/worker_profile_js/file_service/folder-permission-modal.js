...
class FolderPermissionModal {
    ...
    async open(folder, targetCompanyCode, onSave = null) {
        ...
        console.log('📂 İcazə modalı açılır:', {
            ...
            ownCompany: this.ownCompanyCode
        });
    ...
    async loadData() {
        ...
        try {
            ...
            if (employeesResult.success) {
                ...
                console.log('📋 İşçilər:', this.employees);
            } else {
            ...
            console.log('📥 İcazələr yüklənir... Folder ID:', this.folder.id);
            ...
            if (permissionsResult.success) {
                ...
                console.log('📋 İcazələr:', this.permissions);
            } else {
            ...
            console.log('✅ İşçilər icazələrlə birlikdə:', this.employees);
            ...
            <div id="folderPermissionModal" class="fixed inset-0 bg-black/50 z-[9999] flex items-center justify-center p-4" style="backdrop-filter: blur(2px);">
                <div class="bg-white rounded-xl max-w-4xl w-full max-h-[90vh] flex flex-col shadow-2xl">
                    ...
                    <div class="mx-3 mt-3 p-2 bg-blue-50 border border-blue-200 rounded-lg text-xs text-blue-700">
                        ...
                        <strong>${this.ownCompanyCode}</strong> şirkətinin işçilərinə <strong>${this.targetCompanyCode}</strong> şirkətinin "${this.folder.name}" qovluğuna icazə verirsiniz.
                    </div>

                    <!-- Search və Select Buttons -->
                    <div class="p-3 border-b border-gray-200">
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            <button id="selectAllViewBtn" class="px-3 py-2 text-xs bg-purple-100 text-purple-700 rounded-lg hover:bg-purple-200" title="Hamısına görmə icazəsi ver">
                                <i class="fa-solid fa-eye"></i> Hamısını gör
                            </button>
                        </div>
                        <div class="flex gap-2 text-xs">
                            <button id="selectAllBtn" class="px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200">
                                <i class="fa-solid fa-check-double"></i> Hamısını seç
                            </button>
                            <button id="deselectAllBtn" class="px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200">
                                <i class="fa-solid fa-times"></i> Təmizlə
                            </button>
                        </div>
                    </div>

                    <!-- Employees List -->
                    <div class="flex-1 overflow-y-auto p-3" id="permEmployeesList" style="max-height: 350px;">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-[8px] bg-gray-100 px-1.5 py-0.5 rounded-full text-gray-600">ID: ${emp.id || emp.user_id}</span>
                            </div>
    ...
    attachEventListeners() {
        ...
        // Hamısına görmə icazəsi ver
        document.getElementById('selectAllViewBtn')?.addEventListener('click', () => {
            document.querySelectorAll('.perm-checkbox[data-perm="view"]:not([disabled])').forEach(cb => {
                cb.checked = true;
            });
            this.updateSelectedCount();
        });
    ...
    async savePermissions() {
        try {
            ...
            console.log('💾 İcazələr yadda saxlanır...');
            console.log('📌 Folder ID:', this.folder.id);
            console.log('📌 Target Company:', this.targetCompanyCode);
            ...
            console.log('📤 Göndəriləcək icazələr:', permissionsArray);

            // Heç bir checkbox seçilməyibsə xəbərdarlıq et
            if (permissionsArray.length === 0) {
                if (!confirm('Heç bir icazə seçilməyib. Bütün icazələri silmək istəyirsiniz?')) {
                    return;
                }
            }
            ...
            if (result.success) {
                ...
                // ƏN VACİB HİSSƏ: İcazələri yenidən yüklə!
                if (this.service && this.service.loadUserPermissions) {
                    console.log('🔄 İcazələr yenidən yüklənir...');
                    await this.service.loadUserPermissions(this.targetCompanyCode);
                }

                // UI-ı yenilə
                if (window.filesUI && window.filesUI.loadCompanyFiles) {
                    setTimeout(() => {
                        window.filesUI.loadCompanyFiles({ code: this.targetCompanyCode });
                    }, 500);
                }
                ...
                if (window.filesUI?.showNotification) {
                    window.filesUI.showNotification('Xəta: ' + (result.error || 'Bilinməyən xəta'), 'error');
                }
            }
