...
class PermissionsService {
    ...
    loadCurrentUser() {
        try {
            ...
            // Try localStorage.getItem('userData') first
            ...
            // If not found, try localStorage.getItem('user')
            if (!userData || !userData.user) {
                ...
                if (userStr && userStr !== 'null' && userStr !== 'undefined') {
                    try {
                        ...
                        // Check if it has company_id directly
                        if (user && user.company_id) {
                            this.currentUser = user;
                            this.currentCompanyId = user.company_id;
                            console.log('✅ User məlumatları yükləndi (direct user):', {
                        ...
                        // Check if it's in user.user format
                        if (user && user.user && user.user.company_id) {
                            this.currentUser = user.user;
                            this.currentCompanyId = user.user.company_id;
                            console.log('✅ User məlumatları yükləndi (user.user):', {
                                id: this.currentUser.id,
                                companyId: this.currentCompanyId
                            });
                            return;
            ...
            // Process userData if we have it
            if (userData) {
                // Check different possible structures
                if (userData.user && userData.user.company_id) {
                    this.currentUser = userData.user;
                    this.currentCompanyId = userData.user.company_id;
                    console.log('✅ User məlumatları yükləndi (userData.user):', {
                        id: this.currentUser.id,
                        companyId: this.currentCompanyId
                    });
                } else if (userData.company_id) {
                    this.currentUser = userData;
                    this.currentCompanyId = userData.company_id;
                    console.log('✅ User məlumatları yükləndi (direct userData):', {
                        id: this.currentUser.id,
                        companyId: this.currentCompanyId
                    });
                } else if (userData.id && userData.company_id) {
                    this.currentUser = userData;
                    this.currentCompanyId = userData.company_id;
                    console.log('✅ User məlumatları yükləndi (userData with id):', {
                        id: this.currentUser.id,
                        companyId: this.currentCompanyId
                    });
                }
            ...
            // If still no company_id, try to get from API
            if (!this.currentCompanyId && this.api && this.api.hasToken()) {
                console.log('🔄 Company ID tapılmadı, API-dən cəhd edilir...');
                this.fetchUserFromApi();
            }

            // If still no company_id, use default for debugging
            if (!this.currentCompanyId) {
                console.warn('⚠️ Company ID tapılmadı, default 1 istifadə olunur');
    ...
    async fetchUserFromApi() {
        try {
            ...
            console.log('✅ API-dən user cavabı:', response);
            ...
            console.error('❌ API-dən user gətirilə bilmədi:', error);
        }
    }
    ...
    async showPermissionsSection() {
        ...
        try {
            ...
            const protocolNotesSection = document.getElementById('protocolNotesSection');
            if (protocolNotesSection) protocolNotesSection.style.display = 'none';

            let existingSection = document.getElementById('permissionsSection');
            if (!existingSection) {
                this.createPermissionsSection();
            } else {
                this.permissionsSection = existingSection;
            }
            ...
            if (this.permissionsSection) {
                this.permissionsSection.classList.remove('hidden');
    ...
    async getCompanyDepartments(companyId) {
        try {
            ...
            } else if (response && response.departments && Array.isArray(response.departments)) {
                return response.departments;
            } else {
                console.warn('⚠️ Departamentlər gözlənilən formatda deyil:', response);

                // If 403 error, check if user has permission
                if (response && response.status === 403) {
                    this.showErrorMessage('Bu şirkətin departamentlərini görməyə icazəniz yoxdur.');
                }
            ...
            if (error.message && error.message.includes('403')) {
                this.showErrorMessage('İcazə xətası: Departamentləri görmək üçün icazəniz yoxdur.');
            }
