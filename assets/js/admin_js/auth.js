...
async function logoutUser() {
    try {
        ...
        const preservedKeys = Object.keys(localStorage).filter(k => k.startsWith('protocol_read_notes'));
        const preservedData = {};
        preservedKeys.forEach(k => preservedData[k] = localStorage.getItem(k));
        localStorage.clear();
        Object.keys(preservedData).forEach(k => localStorage.setItem(k, preservedData[k]));
        ...
        // Yenə də təmizlə
        const preservedKeysErr = Object.keys(localStorage).filter(k => k.startsWith('protocol_read_notes'));
        const preservedDataErr = {};
        preservedKeysErr.forEach(k => preservedDataErr[k] = localStorage.getItem(k));
        localStorage.clear();
        Object.keys(preservedDataErr).forEach(k => localStorage.setItem(k, preservedDataErr[k]));
        sessionStorage.clear();
        window.location.href = '../login.html';
    }
}