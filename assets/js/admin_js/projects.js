...
// frontend/admin/admin_js/projects.js
    ...
    projectsData.sort((a, b) => (a.order || 999) - (b.order || 999)).forEach((p, i) => {
        html += `<tr data-project-id="${p.id}">
            ...
            <td>
                ...
                <button class="btn btn-sm btn-danger" onclick="deleteProjectItem(${p.id})">  // ✅ deleteProjectItem
...
window.saveProject = async function() {
    ...
    try {
        ...
        // ✅ 1. ƏGƏR FAYL SEÇİLİBƏ - PUBLIC UPLOAD!
        if (window.pendingFile && window.pendingFile.file) {
            ...
            // ✅ FormData yarat - PUBLIC UPLOAD üçün
            const formData = new FormData();
            formData.append('file', file);
            formData.append('category', category);

            // ✅ YENİ ENDPOINT: /public-upload
            const uploadResponse = await fetch('/proxy.php/api/v1/files/public-upload', {
                method: 'POST',
                body: formData,
                credentials: 'include',
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('auth_token') || localStorage.getItem('guven_token')}`
                }
            });
            ...
            console.log('✅ Fayl UUID:', uploadedFileUuid);
            console.log('✅ is_public: TRUE (public-upload)');
...
// ============ SİL ============
window.deleteProjectItem = async function(id) {  // ✅ deleteProject əvəzinə deleteProjectItem
