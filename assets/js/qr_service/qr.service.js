...
class QRCodeService {
    ...
    async generateQrCode() {
        ...
        if (!content) {
            this.showNotification('Zəhmət olmasa məlumatları doldurun', 'warning');
            return;
        }
        ...
        try {
            ...
            if (!userId) {
                try {
                    ...
                    console.warn('⚠️ Cari istifadəçi tapılmadı:', e);
                }
            }

            // ✅ Əgər userId tapılmadısa, xəta ver
            if (!userId) {
                throw new Error('İstifadəçi ID tapılmadı! Zəhmət olmasa səhifəni yeniləyin.');
            }

            console.log('👤 İstifadəçi ID:', userId);

            const qrData = {
                user_id: userId,  // ✅ ƏLAVƏ EDİLDİ
                qr_code_name: name,
                qr_code_type: 'vcard',
                qr_content: content,
            ...
            console.log('📤 QR kod yaradılır:', qrData);
            ...
            const url = `${baseUrl}/api/v1/qr-codes/create`;
            ...
            if (!response.ok) {
                ...
                console.error('❌ Xəta cavabı:', errorText);
            ...
            this.displayQrCode(responseData);
            this.showNotification('✅ QR kod uğurla yaradıldı!', 'success');

        } catch (error) {
            console.error('❌ QR kod xətası:', error);

            // Backend işləmirsə lokal yarat
            try {
                const size = parseInt(document.getElementById('qrCodeSize')?.value) || 500;
                const localQr = await this.createLocalQrCode(content, size);
                this.displayLocalQrCode(localQr, name, 'vcard');
                this.showNotification('⚠️ QR kod lokal olaraq yaradıldı (API bağlantısı yoxdur)', 'warning');
            } catch (localError) {
                console.error('❌ Lokal QR kod xətası:', localError);
                this.showNotification('❌ QR kod yaradıla bilmədi: ' + error.message, 'error');
            }
    ...
    displayQrCode(qrData) {
        ...
        if (image) {
            ...
            // Şəkil yükləndikdə download üçün hazırla
            image.onload = () => {
            ...
            image.onerror = () => {
                ...
                if (downloadBtn) downloadBtn.disabled = false; // Yenə də aktiv et
            };
        }

        if (displayName) displayName.textContent = qrData.qr_code_name || 'QR Kod';
        if (displayId) displayId.textContent = `ID: ${qrData.id || '-'}`;
        if (downloadBtn) downloadBtn.disabled = false;

        // ✅ Əgər artıq backend-də saxlanılıbsa, save düyməsini deaktiv et
        if (saveBtn) {
