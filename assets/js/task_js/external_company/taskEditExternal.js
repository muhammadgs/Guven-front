// taskEditExternal.js - FIXED VERSION for external tasks
const TaskEditExternalModule = {
    // ==================== TASK EDIT MODAL ====================
    openEditTaskModal: async function (taskId, taskType = 'external') {
        try {
            ...
            // 🔥 DÜZƏLİŞ: tasks-external istifadə et
            if (taskType === 'external') {
                endpoint = `/tasks-external/${taskId}`;  // ✅ DÜZGÜN
                    ...
                    <div class="modal-body">
                        ...
                        <form id="taskEditForm">
                            ...
                            <div class="form-actions">
                                ...
                                <button type="button" class="btn btn-primary" onclick="TaskEditExternalModule.saveTaskEdit()">
                                    <i class="fa-solid fa-save"></i> Yadda Saxla
                                </button>
                                <button type="button" class="btn btn-secondary" onclick="TaskEditExternalModule.closeEditModal()">
    ...
    saveTaskEdit: async function() {
        ...
        try {
            ...
            const viewableCompanyEl = document.getElementById('editViewableCompany');
            const isViewableCheckbox = document.getElementById('editIsCompanyViewable');
            ...
            try {
                let endpoint = '';
                if (taskType === 'external') {
                    endpoint = `/tasks-external/${taskId}`;
                } else {
                    endpoint = `/tasks/${taskId}`;
                }
            ...
            if (selectedCompanyId && selectedCompanyId !== '') {
                updateData.company_id = parseInt(selectedCompanyId);
            }

            updateData.is_company_viewable = isViewable;
            if (isViewable && selectedViewableCompanyId && selectedViewableCompanyId !== '') {
                updateData.viewable_company_id = parseInt(selectedViewableCompanyId);
            }
            ...
            if (response && !response.error) {
                ...
                // 🔥 YENİ STATUS "completed" OLDUQDA ARXİVƏ KÖÇÜR
                if (newStatus === 'completed' && taskType === 'external') {
                    console.log('📦 External task tamamlandı, arxivə köçürülür...');

                    try {
                        // Tamamlanmış task məlumatlarını al
                        const completedTask = response.data || response || currentTask;

                        // Arxiv üçün data hazırla
                        const archiveData = {
                            original_task_id: parseInt(taskId),
                            task_code: completedTask.task_code || `EXT-${taskId}`,
                            task_title: completedTask.task_title || taskTitle,
                            task_description: completedTask.task_description || '',
                            assigned_to: completedTask.assigned_to,
                            assigned_by: completedTask.assigned_by,
                            company_id: completedTask.company_id,
                            company_name: completedTask.company_name,
                            department_id: completedTask.department_id,
                            priority: completedTask.priority || 'medium',
                            status: 'completed',
                            due_date: completedTask.due_date,
                            completed_date: new Date().toISOString(),
                            estimated_hours: parseFloat(completedTask.estimated_hours) || 0,
                            actual_hours: parseFloat(completedTask.actual_hours) || 0,
                            work_type_id: completedTask.work_type_id,
                            progress_percentage: 100,
                            is_billable: completedTask.is_billable || false,
                            billing_rate: parseFloat(completedTask.billing_rate) || 0,
                            tags: completedTask.tags,
                            attachments: completedTask.attachments,
                            created_by: completedTask.created_by,
                            creator_name: completedTask.creator_name || userName,
                            started_date: completedTask.started_date,
                            archive_reason: 'External task tamamlandı',
                            target_company_id: completedTask.target_company_id,
                            target_company_name: completedTask.target_company_name
                        };

                        console.log('📦 Archive data:', archiveData);

                        // Archive endpoint-inə göndər
                        const archiveResponse = await makeApiRequest('/task-archive/archive-external', 'POST', archiveData);

                        if (archiveResponse && !archiveResponse.error) {
                            console.log('✅ External task arxivə köçürüldü');
                            alert('✅ External task tamamlandı və arxivə köçürüldü!');
                        } else {
                            console.warn('⚠️ External task arxivə köçürülmədi:', archiveResponse);
                            alert('✅ External task tamamlandı, amma arxivə köçürülmədi');
                        }

                    } catch (archiveError) {
                        console.error('❌ External task arxivləşərkən xəta:', archiveError);
                        alert('✅ External task tamamlandı, amma arxivə köçürülmədi: ' + archiveError.message);
                    }

                } else if (newStatus === 'completed' && taskType !== 'external') {
                    // Daxili task üçün mövcud arxivləşdirmə
                    console.log('✅ Daxili task tamamlandı, arxivləşdirilir...');
                    ...
                    const archiveData = {
                        ...
                        company_id: updateData.company_id || taskDetails.company_id,
                        company_name: taskDetails.company_name || currentUser?.companyName,
                        viewable_company_id: updateData.viewable_company_id || taskDetails.viewable_company_id,
                    ...
                    try {
                        ...
                        if (archiveResponse && !archiveResponse.error) {
                            ...
                            alert('✅ Task yeniləndi, amma arxivə köçürülmədi');
                        }
                    } catch (archiveError) {
...
// Global export
window.TaskEditExternalModule = TaskEditExternalModule;
