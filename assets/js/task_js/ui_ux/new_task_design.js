(function() {
    ...
    function stripCompanyCodeSuffix(name) {
            ...
            if (parentSelect) {
                ...
                if (list.length > 0) {
                    ...
                    sortByVisibleName(list, company => company.company_name || company.name).forEach(company => {
                        ...
                        html += `<option value="${id}" data-company-name="${name}">${name}</option>`;
                    });
    ...
    function populateSelects() {
        ...
        if (companySelect) {
            ...
            if (myCompany && myCompany.id) {
                // Şirkət adını tap
                let companyName = stripCompanyCodeSuffix(myCompany.company_name || myCompany.name);
    ...
    function openModal(taskType) {
        ...
        if (modal) {
            modal.classList.add('internal-task-modal');
        ...
        try {
            ...
            if (currentTaskType === 'internal') {
                ...
                if (companySelect && companySelect.value) {
                    ...
                    selectedCompanyName = stripCompanyCodeSuffix(
                        opt.text.replace('🏢', '').replace('(Mənim şirkətim)', '')
                    );
                }
            }else if (currentTaskType === 'parent') {
                const parentSelect = document.getElementById('newtaskParentSelect');
