...
const ActiveRowCreator = {
    ...
    _syncCompanyName: function(task) {
        // ✅ 1. target_company_name (backend-dən gələn)
        if (task.target_company_name && task.target_company_name.trim()) {
            return task.target_company_name;
        }

        // ✅ 2. viewable_company_name (əgər varsa)
        if (task.viewable_company_name && task.viewable_company_name.trim()) {
            return task.viewable_company_name;
        }

        // ✅ 3. viewable_company_id üzrə cache
        if (task.viewable_company_id) {
            const cache = window.taskManager?.companyCache || {};
            if (cache[task.viewable_company_id]) {
        ...
        // ✅ 4. task_title-dan yoxla (fallback)
        ...
        // ✅ 5. company_name (əsl sahib) - BURADA ƏSAS DÜZƏLİŞ!
