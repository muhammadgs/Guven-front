    ...
    bindSectionEvents() {
        ...
        if (filterSelect) {
            filterSelect.addEventListener('change', (e) => {
                ...
                this.loadEmployees(); // 🔥 API-dan yenidən yüklə
            });
        }
    ...
    async loadEmployees() {
        ...
        try {
            if (!this.currentCompanyCode) {
                console.warn('⚠️ Company code yoxdur');
                return;
            }

            console.log('📥 İşçilər yüklənir:', this.currentCompanyCode);

            // 🔥 NƏ GÖSTƏRƏCƏYİNİ TƏYİN ET
            // - 'all' → bütün işçilər (deaktiv daxil)
            // - 'active' → yalnız aktivlər
            // - 'inactive' → yalnız deaktivlər
            const showInactive = this.filterStatus === 'all' || this.filterStatus === 'inactive';

            // 🔥 API-YA PARAMETR GÖNDƏR
            const url = `/users/company/${this.currentCompanyCode}?include_inactive=${showInactive}`;
            console.log(`🌐 API sorğusu: ${url}`);

            const employees = await this.api.get(url);
            this.employees = Array.isArray(employees) ? employees : [];

            console.log(`✅ ${this.employees.length} işçi tapıldı (include_inactive=${showInactive})`);

            // 2. Departamentləri yüklə
            try {
                const departments = await this.getCompanyDepartments();
                this.departments = Array.isArray(departments) ? departments : [];
            } catch (deptError) {
                console.warn('⚠️ Departamentlər yüklənmədi');
                this.departments = [];
            }

            // 3. Filterləri tətbiq et (frontend-də əlavə filtr)
            this.filterEmployees();
