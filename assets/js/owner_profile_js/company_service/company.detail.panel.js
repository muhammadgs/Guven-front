 ...
 * 4 tabli panel açır:
 *  1. Tam məlumatlar (VÖEN + risk statusu ilə)
...
<div id="cdpModal"></div>
    ...
    _renderList(query = '') {
        ...
        list.innerHTML = filtered.map(c => {
            ...
            if (c.is_risky_payer === true) {
                riskIcon = `<span style="color:#dc2626;font-size:11px;margin-left:4px;" title="Riskli Vergi Ödəyicisi"><i class="fa-solid fa-triangle-exclamation"></i></span>`;
            } else if (c.is_risky_payer === false) {
                riskIcon = `<span style="color:#16a34a;font-size:11px;margin-left:4px;" title="Riskli Deyil"><i class="fa-solid fa-circle-check"></i></span>`;
            }
            ...
            <div class="cdp-item" data-code="${c.company_code}"
              ...
              <div style="min-width:0;">
                <div style="font-size:13px;font-weight:500;color:var(--color-text-primary,#111);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
    ...
    async _selectCompany(code) {
        ...
        // Şirkət məlumatlarını yenilə (VÖEN və risk üçün)
        try {
            if (this.api) {
                const full = await this.api.get(`/companies/${code}/full`);
                if (full) {
                    const idx = this.companies.findIndex(c => c.company_code === code);
                    if (idx !== -1) {
                        this.companies[idx] = { ...this.companies[idx], ...full };
                    }
                }
            }
        } catch (e) {
            console.warn('Full company data yüklənə bilmədi:', e);
        }
...
<div id="cdpTabBody" style="flex:1;overflow-y:auto;padding:16px 20px;"></div>
    ...
    /** ==== TAB 1: MƏLUMATLAR (VÖEN + Risk Statusu) ==== */
    ...
    async _tabInfo() {
        ...
        const rel = full.is_parent ? 'İcraçı Şirkət' : full.is_child ? 'Sifarişçi Şirkət' : '—';
        const f = v => v || '—';
        ...
        if (full.is_risky_payer === true) {
            riskIcon = `<i class="fa-solid fa-triangle-exclamation" style="color:#dc2626;font-size:16px;cursor:pointer;" id="riskIcon" title="Riskli Vergi Ödəyicisi - Ətraflı məlumat üçün klik edin"></i>`;
            riskColor = '#dc2626';
            riskText = 'Riskli';
        } else if (full.is_risky_payer === false) {
            riskIcon = `<i class="fa-solid fa-circle-check" style="color:#16a34a;font-size:16px;cursor:pointer;" id="riskIcon" title="Riskli Deyil - Ətraflı məlumat üçün klik edin"></i>`;
            riskColor = '#16a34a';
        ...
        // ========== VÖEN Ətraflı Məlumatları ==========
        const voenValue = full.voen || '—';
        const vatPayer = full.vat_payer === true ? '✅ Bəli' : full.vat_payer === false ? '❌ Xeyr' : '—';
        const taxpayerStatus = full.taxpayer_status || '—';
        const legalAddress = full.legal_address || '—';
        const legitimate = full.legitimate || '—';
        const organizationType = full.organization_type || '—';
        const charterCapital = full.charter_capital || '—';
        const taxAuthority = full.tax_authority || '—';
        const voenRegisteredAt = full.voen_registered_at ? new Date(full.voen_registered_at).toLocaleDateString('az-AZ') : '—';

        // ========== VOEN Toggle Script ==========
        const voenToggleScript = `
        <script>
        (function() {
            const toggleBtn = document.getElementById('voenToggleBtn');
            const voenDetails = document.getElementById('voenDetails');
            if (toggleBtn && voenDetails) {
                toggleBtn.addEventListener('click', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    if (voenDetails.style.display === 'none' || voenDetails.style.display === '') {
                        voenDetails.style.display = 'block';
                        voenDetails.style.animation = 'slideDown 0.3s ease';
                        this.innerHTML = '<i class="fa-solid fa-chevron-up"></i> Gizlət';
                    } else {
                        voenDetails.style.display = 'none';
                        this.innerHTML = '<i class="fa-solid fa-chevron-down"></i> Ətraflı';
                    }
                });
            }
            ...
            if (riskIcon) {
                riskIcon.addEventListener('click', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    const voenDetails = document.getElementById('voenDetails');
                    const toggleBtn = document.getElementById('voenToggleBtn');
                    if (voenDetails && toggleBtn) {
                        if (voenDetails.style.display === 'none' || voenDetails.style.display === '') {
                            voenDetails.style.display = 'block';
                            voenDetails.style.animation = 'slideDown 0.3s ease';
                            toggleBtn.innerHTML = '<i class="fa-solid fa-chevron-up"></i> Gizlət';
                        }
                    }
                });
            }
...
<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:0.5px solid var(--color-border-tertiary,#e5e7eb);font-size:13px;">
  ...
  <span style="font-weight:500;color:var(--color-text-primary,#111);text-align:right;max-width:200px;word-break:break-word;">${v}</span>
        ...
        // VÖEN sətri - risk ikonu ilə birlikdə
        const voenRow = `
<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:0.5px solid var(--color-border-tertiary,#e5e7eb);font-size:13px;">
  <span style="color:var(--color-text-secondary,#6b7280);display:flex;align-items:center;gap:6px;">
    VÖEN
    ${riskIcon ? `<span style="margin-left:4px;">${riskIcon}</span>` : ''}
    ${riskText ? `<span style="font-size:10px;color:${riskColor};font-weight:500;margin-left:4px;">(${riskText})</span>` : ''}
  </span>
  <span style="font-weight:600;color:#185FA5;font-size:14px;display:flex;align-items:center;gap:8px;">
    ${voenValue}
    <button id="voenToggleBtn">
      <i class="fa-solid fa-chevron-down"></i> Ətraflı
    </button>
  </span>
</div>`;
...
<div style="border:0.5px solid var(--color-border-tertiary,#e5e7eb);border-radius:8px;padding:0 12px;">
  ...
  <!-- VÖEN ƏTRAFLI MƏLUMATLAR (toggle ilə açılan) -->
  <div id="voenDetails" style="display:none;padding:8px 0 12px 0;">
    <div style="background:#f8fafc;border-radius:8px;padding:12px;border:1px solid #e5e7eb;">
      <div style="font-size:11px;font-weight:600;color:#6b7280;margin-bottom:8px;text-transform:uppercase;letter-spacing:0.05em;">
        <i class="fa-solid fa-file-invoice" style="color:#185FA5;margin-right:6px;"></i>
        VÖEN ƏTRAFLI MƏLUMATLAR
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
        <div style="background:white;padding:6px 10px;border-radius:6px;border:1px solid #e5e7eb;">
          <div style="font-size:9px;color:#6b7280;">Status</div>
          <div style="font-size:12px;font-weight:500;color:#111;">${taxpayerStatus}</div>
        </div>
        <div style="background:white;padding:6px 10px;border-radius:6px;border:1px solid #e5e7eb;">
          <div style="font-size:9px;color:#6b7280;">ƏDV ödəyicisi</div>
          <div style="font-size:12px;font-weight:500;color:#111;">${vatPayer}</div>
        </div>
        <div style="background:white;padding:6px 10px;border-radius:6px;border:1px solid #e5e7eb;">
          <div style="font-size:9px;color:#6b7280;">Təşkilat tipi</div>
          <div style="font-size:12px;font-weight:500;color:#111;">${organizationType}</div>
        </div>
        <div style="background:white;padding:6px 10px;border-radius:6px;border:1px solid #e5e7eb;">
          <div style="font-size:9px;color:#6b7280;">Kapital</div>
          <div style="font-size:12px;font-weight:500;color:#111;">${charterCapital} AZN</div>
        </div>
        <div style="background:white;padding:6px 10px;border-radius:6px;border:1px solid #e5e7eb;grid-column:span 2;">
          <div style="font-size:9px;color:#6b7280;">Qeydiyyat tarixi</div>
          <div style="font-size:12px;font-weight:500;color:#111;">${voenRegisteredAt}</div>
        </div>
        <div style="background:white;padding:6px 10px;border-radius:6px;border:1px solid #e5e7eb;grid-column:span 2;">
          <div style="font-size:9px;color:#6b7280;">Rəhbər</div>
          <div style="font-size:12px;font-weight:500;color:#111;">${legitimate}</div>
        </div>
        <div style="background:white;padding:6px 10px;border-radius:6px;border:1px solid #e5e7eb;grid-column:span 2;">
          <div style="font-size:9px;color:#6b7280;">Vergi orqanı</div>
          <div style="font-size:12px;font-weight:500;color:#111;">${taxAuthority}</div>
        </div>
        <div style="background:white;padding:6px 10px;border-radius:6px;border:1px solid #e5e7eb;grid-column:span 2;">
          <div style="font-size:9px;color:#6b7280;">Hüquqi ünvan</div>
