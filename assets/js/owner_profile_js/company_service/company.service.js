...
class CompaniesService {
        ...
        try {
            ...
            try {
                ...
                if (subResponse && subResponse.sub_companies) {
                    ...
                    console.log(`✅ ${subCompanies.length} Sifarişçi yükləndi`);
                }
            } catch (subError) {
            ...
            try {
                ...
                if (subsidiariesResponse && subsidiariesResponse.success && subsidiariesResponse.subsidiaries) {
                    ...
                    console.log(`✅ ${subsidiaryCodes.size} rəsmi Sifarişçi işarələndi`);
                }
            } catch (relError) {
                console.log('ℹ️ Rəsmi sifarişçiər yüklənə bilmədi:', relError.message);
            }

            // ========== 3. Icraçı (parent-companies) ==========
            try {
                ...
                if (parentResponse && parentResponse.success && parentResponse.parent_companies) {
                    ...
                    console.log(`✅ ${parentCompanies.length} Icraçı yükləndi`);
                }
            } catch (parentError) {
                console.log('ℹ️ Icraçı yüklənə bilmədi:', parentError.message);
            }
    ...
    // company.service.js - createTableRow metodunu YENİLƏYİN
    ...
    createTableRow(company) {
        ...
        // ✅ YENİ: Risk statusu
        let riskBadge = '';
        if (company.is_risky_payer === true) {
            riskBadge = `<span class="ml-1 text-red-500" title="Riskli Vergi Ödəyicisi"><i class="fa-solid fa-triangle-exclamation"></i></span>`;
        } else if (company.is_risky_payer === false) {
            riskBadge = `<span class="ml-1 text-green-500" title="Riskli Deyil"><i class="fa-solid fa-circle-check"></i></span>`;
        }
        ...
        if (company.relationship_type === 'parent' || company.is_parent) {
            relationshipIcon = `<i class="fa-solid fa-arrow-up text-blue-600 ml-1" title="İcraçı Şirkət"></i>`;
            relationshipTooltip = 'İcraçı Şirkət';
        } else if (company.relationship_type === 'child' || company.is_child) {
            relationshipIcon = `<i class="fa-solid fa-arrow-down text-green-600 ml-1" title="Sifarişçi Şirkət"></i>`;
            relationshipTooltip = 'Sifarişçi Şirkət';
        }
    ...
    _cdpPanel(company, companyCode) {
        ...
        // ========== YENİ: VÖEN və Risk Statusu ==========
        const voenValue = c.voen || '—';

        // Risk statusunu müəyyən et
        let riskStatus = 'unknown';
        let riskIcon = '';
        let riskColor = '';
        let riskText = '';

        if (c.is_risky_payer === true) {
            riskStatus = 'risky';
            riskIcon = '<i class="fa-solid fa-triangle-exclamation" style="color:#dc2626;font-size:16px;"></i>';
            riskColor = 'bg-red-100 text-red-700 border-red-200';
            riskText = 'Riskli Vergi Ödəyicisi';
        } else if (c.is_risky_payer === false) {
            riskStatus = 'safe';
            riskIcon = '<i class="fa-solid fa-circle-check" style="color:#16a34a;font-size:16px;"></i>';
            riskColor = 'bg-green-100 text-green-700 border-green-200';
            riskText = 'Riskli Deyil';
        } else {
            riskStatus = 'unknown';
            riskIcon = '<i class="fa-solid fa-circle-question" style="color:#6b7280;font-size:16px;"></i>';
            riskColor = 'bg-gray-100 text-gray-600 border-gray-200';
            riskText = 'Məlumat Yoxdur';
        }

        // ========== VÖEN məlumatları üçün əlavə info ==========
        const vatPayer = c.vat_payer === true ? '✅ Bəli' : '❌ Xeyr';
        const taxpayerStatus = c.taxpayer_status || '—';
        const legalAddress = c.legal_address || '—';
        const legitimate = c.legitimate || '—';
        const organizationType = c.organization_type || '—';
        const charterCapital = c.charter_capital || '—';
        const taxAuthority = c.tax_authority || '—';
        ...
        .risk-badge{display:inline-flex;align-items:center;gap:6px;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;border:1px solid;}
        </style>
        ...
        <div style="background:#fff;border:.5px solid #e5e7eb;border-radius:12px;padding:20px 24px;margin-bottom:14px;">
            <div style="display:flex;align-items:center;gap:14px;margin-bottom:16px;flex-wrap:wrap;">
                ...
                <div style="flex:1;min-width:0;">
                    ...
                    <div style="font-size:12px;color:#6b7280;margin-top:3px;display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
                        <span>${companyCode}</span>
                        ${c.voen ? `<span>· VÖEN: ${c.voen}</span>` : ''}
                        ${c.is_parent ? '<span style="background:#EFF6FF;color:#185FA5;padding:2px 8px;border-radius:10px;font-size:10px;">Icraçı</span>' : ''}
                        ${c.is_child ? '<span style="background:#ECFDF5;color:#065F46;padding:2px 8px;border-radius:10px;font-size:10px;">Sifarişçi</span>' : ''}
                    </div>
                </div>
                <div style="display:flex;align-items:center;gap:8px;flex-shrink:0;">
                    <!-- ✅ YENİ: Risk Status Badge -->
            ...
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;">
                ...
                <div class="cdp-stat"><div style="font-size:11px;color:#6b7280;margin-bottom:4px;">Əlaqə növü</div><div style="font-size:13px;font-weight:500;color:#111;">${c.is_parent?'Icraçı':c.is_child?'Sifarişçi':'—'}</div></div>
            ...
            <!-- ✅ YENİ: VÖEN ƏTRAFLI MƏLUMATLAR (əgər varsa) -->
            ${c.voen ? `
            <div style="margin-top:16px;padding:16px;background:#f8fafc;border-radius:12px;border:1px solid #e5e7eb;">
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;">
                    <i class="fa-solid fa-file-invoice" style="color:#185FA5;"></i>
                    <span style="font-weight:600;font-size:14px;color:#111;">VÖEN Məlumatları</span>
                    <span style="font-size:11px;color:#6b7280;background:#e5e7eb;padding:2px 10px;border-radius:10px;">${c.voen}</span>
                </div>
                <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px;">
                    <div style="background:white;padding:8px 12px;border-radius:8px;border:1px solid #e5e7eb;">
                        <div style="font-size:10px;color:#6b7280;">Status</div>
                        <div style="font-size:13px;font-weight:500;color:#111;">${taxpayerStatus}</div>
                    </div>
                    <div style="background:white;padding:8px 12px;border-radius:8px;border:1px solid #e5e7eb;">
                        <div style="font-size:10px;color:#6b7280;">ƏDV ödəyicisi</div>
                        <div style="font-size:13px;font-weight:500;color:#111;">${vatPayer}</div>
                    </div>
                    <div style="background:white;padding:8px 12px;border-radius:8px;border:1px solid #e5e7eb;">
                        <div style="font-size:10px;color:#6b7280;">Təşkilat tipi</div>
                        <div style="font-size:13px;font-weight:500;color:#111;">${organizationType}</div>
                    </div>
                    <div style="background:white;padding:8px 12px;border-radius:8px;border:1px solid #e5e7eb;">
                        <div style="font-size:10px;color:#6b7280;">Nizamnamə kapitalı</div>
                        <div style="font-size:13px;font-weight:500;color:#111;">${charterCapital} AZN</div>
                    </div>
                    <div style="background:white;padding:8px 12px;border-radius:8px;border:1px solid #e5e7eb;grid-column:span 2;">
                        <div style="font-size:10px;color:#6b7280;">Hüquqi ünvan</div>
                        <div style="font-size:12px;font-weight:500;color:#111;">${legalAddress}</div>
                    </div>
                    <div style="background:white;padding:8px 12px;border-radius:8px;border:1px solid #e5e7eb;grid-column:span 2;">
                        <div style="font-size:10px;color:#6b7280;">Vergi orqanı</div>
                        <div style="font-size:12px;font-weight:500;color:#111;">${taxAuthority}</div>
                    </div>
                </div>
            </div>
            ` : ''}
            ...
            <div id="companyCodeModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center" style="z-index: 9999;">
                <div class="bg-white rounded-2xl w-full max-w-md mx-4 shadow-2xl">
                    <div class="px-8 py-6 border-b">
                        <div class="flex items-center justify-between">
                            <div>
                                <h3 class="text-xl font-bold text-gray-900">Mövcud Şirkəti Sifarişçi Et</h3>
    ...
    async checkCompanyInfo(companyCode) {
        ...
        try {
            ...
            if (companyCode === userCompanyCode) {
                validationDiv.innerHTML = '<span class="text-red-500">❌ Öz şirkətinizi Sifarişçi kimi əlavə edə bilməzsiniz</span>';
            ...
            try {
                ...
                if (infoBox && infoContent) {
                    ...
                    infoContent.innerHTML = `
                        ...
                        <p class="text-xs text-amber-700">Bu kodu yenə də Sifarişçi kimi əlavə edə bilərsiniz</p>
                    `;
                }
    ...
    /**
     * ŞİRKƏT KODU İLƏ Sifarişçi ƏLAVƏ ET
     */
    async handleAddCompanyByCode() {
        ...
        try {
            ...
            if (childCompanyCode === userCompanyCode) {
                throw new Error('Öz şirkətinizi Sifarişçi kimi əlavə edə bilməzsiniz');
            }
            ...
            if (this.apiService) {
                try {
                    ...
                    addSuccess = true;
                    addMessage = `"${childCompanyCode}" kodlu şirkət Sifarişçi kimi əlavə edildi (lokal)`;
            ...
            if (submitBtn) {
                ...
                submitBtn.innerHTML = '<i class="fa-solid fa-link mr-2"></i>Sifarişçi Et';
            }
            if (validationDiv) validationDiv.innerHTML = '';
            if (infoBox) infoBox.classList.add('hidden');
        }
    ...
    async exportCompanies() {
        try {
            ...
            const rows = exportData.map(c => [
                ...
                c.email || '—',
                c.phone || '—',
                c.relationship_type === 'parent' ? 'İcraçı Şirkət' :
                c.relationship_type === 'child' ? 'Sifarişçi Şirkət' : '—'
            ]);
