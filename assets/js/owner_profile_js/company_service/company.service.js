...
class CompaniesService {
    ...
    createTableRow(company) {
        ...
        if (company.relationship_type === 'parent' || company.is_parent) {
            relationshipIcon = `<i class="fa-solid fa-arrow-up text-blue-600 ml-1" title="Icraçı"></i>`;
            relationshipTooltip = 'Icraçı';
        } else if (company.relationship_type === 'child' || company.is_child) {
            relationshipIcon = `<i class="fa-solid fa-arrow-down text-green-600 ml-1" title="Sifarişçi"></i>`;
            relationshipTooltip = 'Sifarişçi';
        }
    ...
    async exportCompanies() {
        try {
            ...
            const rows = exportData.map(c => [
                ...
                c.email || '—',
                c.phone || '—',
                c.relationship_type === 'parent' ? 'Icraçı' :
                c.relationship_type === 'child' ? 'Sifarişçi' : '—'
            ]);
