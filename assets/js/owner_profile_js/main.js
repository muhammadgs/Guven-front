...
class ProfileApp {
                                ...
                                <div class="protocol-list-card">
                                    <div class="protocol-card-title-row">
                                        ...
                                        <div>
                                            ...
                                            <p>Cavab gözləyən pratakollar</p>
                                        </div>
                                    </div>
    ...
    renderProtocolEmployeeSelectList() {
        ...
        list.innerHTML = employees.length ? employees.map(emp => `<label class="protocol-employee-select-row protocol-employee-row"><input class="protocol-employee-checkbox" type="checkbox" value="${this.escapeProtocolHtml(emp.id)}"><span class="protocol-participant-avatar protocol-employee-avatar">${this.escapeProtocolHtml(emp.fullName.charAt(0).toUpperCase())}</span><span class="protocol-employee-info"><strong class="protocol-employee-name">${this.escapeProtocolHtml(emp.fullName)}</strong><small class="protocol-employee-department">${this.escapeProtocolHtml(emp.department || 'Şöbə qeyd edilməyib')}</small></span></label>`).join('') : '<div class="protocol-empty-state">Əməkdaş tapılmadı.</div>';
    }
    ...
    injectProtocolNotesStyles() {
        ...
        style.textContent = `
            ...
            .protocol-full-height-mode{min-height:calc(100vh - 32px)}
            #protocolNotesSection{min-height:calc(100vh - 32px);display:flex;flex-direction:column;box-sizing:border-box}
            .protocol-notes-page.employees-like-layout{position:relative;isolation:isolate;overflow:hidden;min-height:calc(100vh - 96px);display:flex;flex-direction:column;box-sizing:border-box;padding:28px 30px 32px!important;border-radius:36px;background:radial-gradient(circle at 12% 2%,rgba(255,255,255,.94),transparent 30%),radial-gradient(circle at 92% 0%,rgba(67,136,255,.14),transparent 26%),linear-gradient(145deg,rgba(247,247,247,.76),rgba(221,228,238,.62));border:1px solid rgba(255,255,255,.68);box-shadow:inset 0 2px 1px rgba(255,255,255,.82),inset 0 -20px 50px rgba(255,255,255,.3),0 0 0 9px rgba(255,255,255,.14),0 28px 78px rgba(15,23,42,.15);backdrop-filter:blur(26px) saturate(165%)}
