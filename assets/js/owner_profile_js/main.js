...
class ProfileApp {
            ...
            newBtn.addEventListener('click', async (e) => {
                ...
                protocolNotesSection.innerHTML = `
                    <div class="protocol-notes-page employees-like-layout">
                        ...
                        <div id="notesPageView" class="protocol-page-view hidden protocol-editor-view">
                            <div class="protocol-page-top protocol-editor-top">
                                <button class="protocol-back-btn" data-notes-page-back type="button">
                                    <i class="fas fa-arrow-left"></i>
                                    Geri
                                </button>
                                <div class="protocol-editor-title">
                                    <h2>Qeydlər</h2>
                                    <p>Professional qeyd yarat, saxla və göndər.</p>
                                </div>
                                <div class="protocol-editor-top-actions">
                                    <button id="openSavedNotesModalBtn" class="protocol-editor-icon-btn" type="button" title="Yadda saxlanan qeydlər" aria-label="Yadda saxlanan qeydlər"><i class="fas fa-bookmark"></i></button>
                                    <button id="openUnsavedNotesModalBtn" class="protocol-editor-icon-btn protocol-editor-trash-btn" type="button" title="Yadda saxlanmamış qeydlər" aria-label="Yadda saxlanmamış qeydlər"><i class="fas fa-trash-alt"></i></button>
                                </div>
                            </div>

                            <div class="protocol-note-editor-card">
                                <div class="protocol-note-form-grid">
                                    <label class="protocol-note-field"><span>Tarix</span><input id="protocolNoteDate" type="text" readonly /></label>
                                    <label class="protocol-note-field"><span>Əməkdaş</span><input id="protocolNoteAuthor" type="text" readonly /></label>
                                </div>
                                <label class="protocol-note-field protocol-note-title-field"><span>Başlıq</span><input id="protocolNoteTitle" type="text" placeholder="Qeyd başlığını yazın..." /></label>
                                <div class="protocol-rich-editor" id="protocolRichEditorWrap">
                                    <div class="protocol-editor-toolbar">
                                        <input id="protocolNoteColor" class="protocol-color-picker" type="color" value="#1f2937" title="Yazı rəngi" aria-label="Yazı rəngi" />
                                        <select id="protocolNoteFont" title="Font seç" aria-label="Font seç"><option value="Inter">Inter</option><option value="Arial">Arial</option><option value="Georgia">Georgia</option><option value="Times New Roman">Times</option><option value="Courier New">Courier</option></select>
                                        <select id="protocolNoteFontSize" title="Şrift ölçüsü" aria-label="Şrift ölçüsü"><option value="3">Normal</option><option value="4">Orta</option><option value="5">Böyük</option><option value="6">Çox böyük</option></select>
                                        <button type="button" data-note-command="underline" title="Underline"><i class="fas fa-underline"></i></button>
                                        <button type="button" data-note-command="italic" title="Italic"><i class="fas fa-italic"></i></button>
                                        <button type="button" data-note-command="bold" title="Bold"><i class="fas fa-bold"></i></button>
                                        <button type="button" data-note-command="justifyLeft" title="Sola düzlə"><i class="fas fa-align-left"></i></button>
                                        <button type="button" data-note-command="justifyCenter" title="Ortala"><i class="fas fa-align-center"></i></button>
                                        <button type="button" data-note-command="justifyRight" title="Sağa düzlə"><i class="fas fa-align-right"></i></button>
                                    </div>
                                    <div id="protocolNoteContent" class="protocol-editor-content" contenteditable="true" data-placeholder="Qeydin mətnini yazın..."></div>
                                </div>
                                <div id="protocolNoteFeedback" class="protocol-note-feedback" aria-live="polite"></div>
                                <div class="protocol-note-actions">
                                    <button id="saveDraftNoteBtn" class="protocol-secondary-btn" type="button"><i class="fas fa-bookmark"></i> Yadda saxla</button>
                                    <button id="sendProtocolNoteBtn" class="protocol-save-btn" type="button"><i class="fas fa-paper-plane"></i> Göndər</button>
                                </div>
                            </div>
                        </div>

                        <div id="protocolSavedNotesModal" class="protocol-notes-archive-modal hidden"><div class="protocol-notes-archive-card"><div class="protocol-modal-header"><h3>Yadda saxlanan qeydlər</h3><button data-close-notes-modal type="button"><i class="fas fa-times"></i></button></div><div id="protocolSavedNotesList" class="protocol-notes-archive-list"></div></div></div>
                        <div id="protocolUnsavedNotesModal" class="protocol-notes-archive-modal hidden"><div class="protocol-notes-archive-card"><div class="protocol-modal-header"><h3>Yadda saxlanmamış qeydlər</h3><button data-close-notes-modal type="button"><i class="fas fa-times"></i></button></div><div id="protocolUnsavedNotesList" class="protocol-notes-archive-list"></div></div></div>
                        <div id="protocolNoteDeleteConfirmModal" class="protocol-delete-confirm-overlay hidden"><div class="protocol-delete-confirm-modal"><div class="protocol-delete-confirm-icon"><i class="fas fa-trash-alt"></i></div><h3>Bu qeyd silinsin?</h3><div class="protocol-delete-confirm-actions"><button id="confirmDeleteNoteYes" class="protocol-delete-confirm-yes" type="button">Bəli</button><button id="confirmDeleteNoteNo" class="protocol-delete-confirm-no" type="button">Xeyr</button></div></div></div>
                    </div>
                `;
    ...
    async initProtocolNotesSection() {
        ...
        document.getElementById('openNotesPageBtn')?.addEventListener('click', () => { showView(notesPage); this.initNotesEditorPage(); });
        ...
        document.querySelector('[data-notes-page-back]')?.addEventListener('click', () => { this.captureUnsavedNote(); showView(protocolListView); this.renderProtocolLists(); });
    ...
    getProtocolNotesStorageKey(type = 'saved') { return `gf44_protocol_${type}_notes`; }

    initNotesEditorPage() {
        const user = this.getCurrentUserForProtocol();
        this.noteDate = this.getTodayDateAz();
        this.noteAuthor = user.name || 'Sadiq Ceferov';
        this.noteTitle = '';
        this.noteContent = '';
        this.savedNotes = this.loadProtocolNotes('saved');
        this.unsavedNotes = this.loadProtocolNotes('unsaved');
        this.selectedNoteForDelete = null;
        const dateEl = document.getElementById('protocolNoteDate');
        const authorEl = document.getElementById('protocolNoteAuthor');
        const titleEl = document.getElementById('protocolNoteTitle');
        const contentEl = document.getElementById('protocolNoteContent');
        if (dateEl) dateEl.value = this.noteDate;
        if (authorEl) authorEl.value = this.noteAuthor;
        if (titleEl) titleEl.value = '';
        if (contentEl) contentEl.innerHTML = '';
        this.setProtocolNoteFeedback('');
    }

    bindNotesEditorEvents() {
        document.getElementById('protocolNoteTitle')?.addEventListener('input', e => { this.noteTitle = e.target.value || ''; });
        document.getElementById('protocolNoteContent')?.addEventListener('input', e => { this.noteContent = e.currentTarget.innerHTML || ''; });
        document.querySelectorAll('[data-note-command]').forEach(btn => btn.addEventListener('click', () => this.applyProtocolNoteCommand(btn.dataset.noteCommand)));
        document.getElementById('protocolNoteColor')?.addEventListener('input', e => this.applyProtocolNoteCommand('foreColor', e.target.value));
        document.getElementById('protocolNoteFont')?.addEventListener('change', e => this.applyProtocolNoteCommand('fontName', e.target.value));
        document.getElementById('protocolNoteFontSize')?.addEventListener('change', e => this.applyProtocolNoteCommand('fontSize', e.target.value));
        document.getElementById('saveDraftNoteBtn')?.addEventListener('click', () => this.saveDraftProtocolNote());
        document.getElementById('sendProtocolNoteBtn')?.addEventListener('click', () => this.sendProtocolNote());
        document.getElementById('openSavedNotesModalBtn')?.addEventListener('click', () => this.openProtocolNotesModal('saved'));
        document.getElementById('openUnsavedNotesModalBtn')?.addEventListener('click', () => this.openProtocolNotesModal('unsaved'));
        document.querySelectorAll('[data-close-notes-modal]').forEach(btn => btn.addEventListener('click', () => this.closeProtocolNotesModals()));
        document.getElementById('confirmDeleteNoteNo')?.addEventListener('click', () => this.closeNoteDeleteConfirmModal());
        document.getElementById('confirmDeleteNoteYes')?.addEventListener('click', () => this.confirmDeleteProtocolNote());
    }

    applyProtocolNoteCommand(command, value = null) {
        document.getElementById('protocolNoteContent')?.focus();
        document.execCommand(command, false, value);
        this.noteContent = document.getElementById('protocolNoteContent')?.innerHTML || '';
    }

    loadProtocolNotes(type) { try { return JSON.parse(localStorage.getItem(this.getProtocolNotesStorageKey(type)) || '[]'); } catch (e) { return []; } }
    saveProtocolNotes(type, notes) { localStorage.setItem(this.getProtocolNotesStorageKey(type), JSON.stringify(notes)); }
    stripProtocolNoteHtml(html = '') { const div = document.createElement('div'); div.innerHTML = html; return (div.textContent || div.innerText || '').trim(); }
    hasCurrentProtocolNoteDraft() { return (this.noteTitle || '').trim() || this.stripProtocolNoteHtml(this.noteContent || '').trim(); }
    buildCurrentProtocolNote() { return { id: `note_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`, date: this.noteDate || this.getTodayDateAz(), author: this.noteAuthor || this.getCurrentUserForProtocol().name, title: (this.noteTitle || '').trim() || 'Başlıqsız qeyd', content: this.noteContent || '', createdAt: new Date().toISOString() }; }

    resetProtocolNoteEditor() {
        this.noteTitle = ''; this.noteContent = ''; this.noteDate = this.getTodayDateAz();
        const dateEl = document.getElementById('protocolNoteDate'); if (dateEl) dateEl.value = this.noteDate;
        const titleEl = document.getElementById('protocolNoteTitle'); if (titleEl) titleEl.value = '';
        const contentEl = document.getElementById('protocolNoteContent'); if (contentEl) contentEl.innerHTML = '';
    }

    captureUnsavedNote() {
        this.noteTitle = document.getElementById('protocolNoteTitle')?.value || this.noteTitle || '';
        this.noteContent = document.getElementById('protocolNoteContent')?.innerHTML || this.noteContent || '';
        if (!this.hasCurrentProtocolNoteDraft()) return;
        const notes = this.loadProtocolNotes('unsaved');
        notes.unshift(this.buildCurrentProtocolNote());
        this.saveProtocolNotes('unsaved', notes);
        this.unsavedNotes = notes;
        this.resetProtocolNoteEditor();
    }

    saveDraftProtocolNote() {
        this.noteTitle = document.getElementById('protocolNoteTitle')?.value || '';
        this.noteContent = document.getElementById('protocolNoteContent')?.innerHTML || '';
        if (!this.hasCurrentProtocolNoteDraft()) { this.setProtocolNoteFeedback('Boş qeyd yadda saxlanılmadı.', 'error'); return; }
        const notes = this.loadProtocolNotes('saved'); notes.unshift(this.buildCurrentProtocolNote());
        this.saveProtocolNotes('saved', notes); this.savedNotes = notes; this.resetProtocolNoteEditor();
        this.setProtocolNoteFeedback('Qeyd yadda saxlandı.', 'success');
    }

    async sendProtocolNote() {
        this.noteTitle = document.getElementById('protocolNoteTitle')?.value || '';
        this.noteContent = document.getElementById('protocolNoteContent')?.innerHTML || '';
        if (!this.stripProtocolNoteHtml(this.noteContent).trim()) { this.setProtocolNoteFeedback('Kontent boş ola bilməz.', 'error'); return; }
        const protocol = this.getProtocolById(this.currentProtocolId) || this.loadProtocols()[0];
        if (!protocol?.id) { this.setProtocolNoteFeedback('Göndərmək üçün pratakol tapılmadı.', 'error'); return; }
        const payloadContent = `<h3>${this.escapeProtocolHtml((this.noteTitle || '').trim() || 'Başlıqsız qeyd')}</h3>${this.noteContent}`;
        const btn = document.getElementById('sendProtocolNoteBtn'); this.setProtocolButtonLoading(btn, true, 'Göndərilir...');
        try {
            await this.getProtocolService().addNote(protocol.id, payloadContent, 0);
            this.setProtocolNoteFeedback('Qeyd göndərildi.', 'success');
            this.resetProtocolNoteEditor();
        } catch (error) { this.setProtocolNoteFeedback(this.getProtocolApiError(error), 'error'); }
        finally { this.setProtocolButtonLoading(btn, false); }
    }

    setProtocolNoteFeedback(message, type = '') { const el = document.getElementById('protocolNoteFeedback'); if (el) { el.textContent = message || ''; el.className = `protocol-note-feedback ${type}`; } }

    openProtocolNotesModal(type) {
        this.renderProtocolNotesArchive(type);
        document.getElementById(type === 'saved' ? 'protocolSavedNotesModal' : 'protocolUnsavedNotesModal')?.classList.remove('hidden');
    }
    closeProtocolNotesModals() { document.getElementById('protocolSavedNotesModal')?.classList.add('hidden'); document.getElementById('protocolUnsavedNotesModal')?.classList.add('hidden'); }

    renderProtocolNotesArchive(type) {
        const list = document.getElementById(type === 'saved' ? 'protocolSavedNotesList' : 'protocolUnsavedNotesList'); if (!list) return;
        const notes = this.loadProtocolNotes(type);
        list.innerHTML = notes.length ? notes.map(note => `<div class="protocol-note-archive-item"><div><div class="protocol-note-archive-title">${this.escapeProtocolHtml(note.title || 'Başlıqsız qeyd')}</div><div class="protocol-note-archive-meta"><span><i class="fas fa-calendar"></i>${this.escapeProtocolHtml(note.date || '')}</span><span><i class="fas fa-user"></i>${this.escapeProtocolHtml(note.author || '')}</span></div><p>${this.escapeProtocolHtml(this.stripProtocolNoteHtml(note.content || '').slice(0, 130) || 'Kontent yoxdur.')}</p></div><button class="protocol-note-delete-btn" data-delete-note-type="${type}" data-delete-note-id="${this.escapeProtocolHtml(note.id)}" type="button"><i class="fas fa-trash-alt"></i></button></div>`).join('') : '<div class="protocol-empty-state">Qeyd yoxdur.</div>';
        list.querySelectorAll('[data-delete-note-id]').forEach(btn => btn.addEventListener('click', () => this.openNoteDeleteConfirmModal(btn.dataset.deleteNoteType, btn.dataset.deleteNoteId)));
    }
    openNoteDeleteConfirmModal(type, id) { this.selectedNoteForDelete = { type, id }; document.getElementById('protocolNoteDeleteConfirmModal')?.classList.remove('hidden'); }
    closeNoteDeleteConfirmModal() { document.getElementById('protocolNoteDeleteConfirmModal')?.classList.add('hidden'); this.selectedNoteForDelete = null; }
    confirmDeleteProtocolNote() { const s = this.selectedNoteForDelete; if (!s) return; const notes = this.loadProtocolNotes(s.type).filter(n => String(n.id) !== String(s.id)); this.saveProtocolNotes(s.type, notes); this.closeNoteDeleteConfirmModal(); this.renderProtocolNotesArchive(s.type); }
