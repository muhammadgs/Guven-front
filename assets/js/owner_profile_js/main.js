...
class ProfileApp {
                                ...
                                <div class="notes-rich-editor-card">
                                    <div class="notes-editor-toolbar" aria-label="Mətn formatlama paneli">
                                        <div class="notes-color-control" title="Hərfin rəngi">
                                            <button id="noteColorToggle" class="notes-color-toggle" type="button" aria-label="Yazı rəngi seç" aria-haspopup="true" aria-expanded="false"><i class="fas fa-palette"></i></button>
                                            <div id="noteColorPopover" class="note-color-popover hidden">
                                                <label class="notes-color-picker"><span>Rəng seç</span><input id="noteTextColor" type="color" value="#111827" aria-label="Yazı rəngi seç" /></label>
                                                <div class="note-color-quick-section">
                                                    <div class="note-color-quick-title">Hazır rənglər</div>
                                                    <div class="note-color-quick-row" aria-label="Hazır yazı rəngləri">
                                                        ${[
                                                        ...
                                                        ].map(({ color, title }) => `<button class="note-color-swatch${color === '#111827' ? ' active' : ''}" type="button" data-color="${color}" title="${title}" aria-label="${title} rəngi" aria-pressed="${color === '#111827'}" style="background:${color}"></button>`).join('')}
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
    ...
    bindStandaloneNoteToolbar() {
        ...
        const colorToggle = document.getElementById('noteColorToggle');
        ...
        colorToggle?.addEventListener('click', (event) => {
            ...
            const willOpen = colorPopover?.classList.contains('hidden');
            colorPopover?.classList.toggle('hidden', !willOpen);
            colorToggle.setAttribute('aria-expanded', willOpen ? 'true' : 'false');
        });
