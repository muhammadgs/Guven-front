...
class ProfileApp {
    ...
    async openCreateProtocolModal() {
        ...
        try {
            ...
            try {
                // ✅ BURANIN YERİNƏ - DÜZGÜN ENDPOINT
                // Köhnə: await this.getProtocolService().getAvailableEmployees(this.currentProtocolId);
                // YENİ: Birbaşa users endpoint
                const companyCode = this.currentCompanyCode || this._getCompanyCodeFromStorage();
                const response = await this.getProtocolService().get(`/users/company/${companyCode}`);
                const employees = Array.isArray(response) ? response : (response?.data || response?.items || []);
                this.protocolEmployeesCache = employees.map(emp => this.mapProtocolEmployee(emp));
    ...
    // ✅ KÖMƏKÇİ METOD
    _getCompanyCodeFromStorage() {
        try {
            const userData = JSON.parse(localStorage.getItem('userData') || '{}');
            return userData.user?.company_code || userData.company_code || userData.companyCode || null;
        } catch (e) {
            return null;
        }
    }
    ...
    renderProtocolNotesPage() {
        ...
        notesCard.innerHTML = notes.length ? `
            <div class="protocol-card-header protocol-section-title-row">
                ...
                <div><h3>Qeydlər</h3><p>Protokol qeydləri</p></div>
            </div>
            <div class="protocol-worker-notes-list">
                ${notes.map(item => `<div class="protocol-worker-note-item">
                    <div class="protocol-list-item-title">${this.escapeProtocolHtml(item.protocol.title || 'Protokol')}</div>
