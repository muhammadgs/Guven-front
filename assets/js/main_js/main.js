...
function loadPartners() {
    console.log('🔄 Partnyorlar API-dən yüklənir...');

    if (window.ApiMainService && window.ApiMainService.partners && typeof window.ApiMainService.partners.getPublic === 'function') {
        window.ApiMainService.partners.getPublic().then(function (result) {
            if (result && result.success && Array.isArray(result.data) && result.data.length) {
...
function loadPartnersFromLocalStorage() {
    ...
    if (savedPartners) {
        try {
            renderPartners(JSON.parse(savedPartners));
            return;
        } catch (error) {
            console.error('❌ JSON parse xətası:', error);
        }
    }
    // Heç bir mənbədən data gəlmirsə
    const container = document.getElementById('partners-container');
    if (container) container.innerHTML = '<p class="empty-msg">Heç bir partnyor tapılmadı</p>';
}
...
function renderPartners(partners) {
    ...
    // ✅ active sahəsi olmasa da göstər, yalnız açıq şəkildə false olanları at
...
function loadProjectsFromLocalStorage() {
    console.log('📂 Layihələr localStorage-dan yüklənir...');
    ...
    if (container) container.innerHTML = '<p class="empty-msg">Heç bir layihə tapılmadı</p>';
}
