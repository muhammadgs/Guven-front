...
class ApiService {
    ...
    resolveBaseUrl() {
        // Backend CORS currently allows only this exact development origin.
        // All other local ports/hosts must go through the web proxy.
        if (this.canUseDirectDevelopmentApi()) {
    ...
    get notes() {
        return {
            /**
             * Yeni note yarat
             * @param {Object} data - { title, content, note_type, color, is_pinned, is_archived, shared_with, metadata }
             * @returns {Promise}
             */
            create: (data) => this.post('/notes/', data),

            /**
             * Notların siyahısını gətir
             * @param {Object} params - { page, limit, note_type, is_pinned, is_archived, search }
             * @returns {Promise}
             */
            list: (params = {}) => {
                ...
                const url = query.toString() ? `/notes/?${query}` : '/notes/';
                return this.get(url);
            },
             ...
             * @returns {Promise}
             */
            update: (id, data) => this.put(`/notes/${id}`, data),

            /**
             * Note sil (soft delete)
             * @param {number|string} id
             * @returns {Promise}
             */
            ...
            /**
             * Cari istifadəçi ilə paylaşılan notlar
             * @returns {Promise}
             */
            shared: () => this.get('/notes/shared'),

            /**
             * Note-u digər istifadəçilərlə paylaş
             * @param {number|string} noteId
             * @param {Object} data - { shared_to: [], permissions: { can_edit, can_comment, can_share } }
             * @returns {Promise}
             */
            share: (noteId, data) => this.post(`/notes/${noteId}/share`, data),

            /**
             * Note şərhlərini gətir
             * @param {number|string} noteId
             * @returns {Promise}
             */
            comments: (noteId) => this.get(`/notes/${noteId}/comments`),

            /**
             * Note-a şərh əlavə et
             * @param {number|string} noteId
             * @param {Object} data - { comment, parent_id }
             * @returns {Promise}
             */
             ...
             * @returns {Promise}
             */
            deleteComment: (noteId, commentId) => this.delete(`/notes/${noteId}/comments/${commentId}`)
        };
    }
    ...
    async getProtocols(filters = {}) {
        ...
        // ✅ Sonda slash əlavə edin: /protocols/ əvəzinə /protocols
    ...
    async searchUsersForProtocol(search, companyCode = null) {
        // ✅ DÜZGÜN ENDPOINT: companyCode ilə users endpoint
        let endpoint = `/users/company/${companyCode}`;

        // ✅ Əgər search varsa, əlavə parametr göndər
    ...
    async addParticipant(protocolId, userId) {
        // ✅ userId array ola bilər
    ...
    async getRecentNotifications(limit = 10) {
        return this.get(`/notifications/recent?limit=${limit}`);
    }
    ...
    async archiveNotification(id) {
        return this.request(`/notifications/${id}/archive`, 'PATCH');
    }
...
window.makeApiRequest = async function(endpoint, method = 'GET', data = null, isFormData = false) {
    const noBaza = _NO_BAZA_PATTERNS.some(p => endpoint.includes(p)) ||
                   endpoint.startsWith('/notes/') || endpoint.startsWith('/notifications/');
